...
namespace VendasWeb.Data
{
    public class SeedingService
    {
        ...
        public SeedingService(VendasWebContext context) /*Injeção de dependencia, quando um SeedingService for criado ele recebe automáticamente 
             um context através da injeção
            */
        {
            _context = context;
        }

        public void Seed() // Adiciona coisas (popula) a base de dados
        {
            if (_context.Department.Any() || _context.Seller.Any() || _context.SalesRecords.Any()) //Verificação se o banco tem dados
            {
                return; // O DB ja foi populado (seeded)
            }

            //Criação de objetos e manda pro banco = "CODE-FIRST workflow"
            ...
            // Adicionando ao DB através do AddRange que permiti adicionar vários objetos de uma vez
            ...
            _context.SaveChanges(); //Salva e confirma as alterações no DB
